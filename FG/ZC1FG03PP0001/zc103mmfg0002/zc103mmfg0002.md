```abap
FUNCTION zc103mmfg0002.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(IV_PREFIX) TYPE  CHAR07
*"     REFERENCE(IV_OBJECT) TYPE  NRIV-OBJECT
*"     REFERENCE(IV_NRO_NO) TYPE  NRIV-NRRANGENR
*"  EXPORTING
*"     REFERENCE(EV_ID) TYPE  CHAR08
*"----------------------------------------------------------------------

  DATA: lv_id         TYPE num10,
        lv_prefix_len TYPE i,
        lv_id_str     TYPE string,
        lv_num_part   TYPE string,
        lv_start      TYPE i,
        lv_len        TYPE i.
  ...
  CALL FUNCTION 'NUMBER_GET_NEXT'
    EXPORTING
      nr_range_nr = iv_nro_no
      object      = iv_object
    IMPORTING
      number      = lv_id.

    " 문자열 변환
  lv_id_str = lv_id. " num10 → string 자동 변환

  " prefix 길이만큼 계산
  lv_prefix_len = strlen( iv_prefix ).
  lv_len = 8 - lv_prefix_len.
  lv_start = strlen( lv_id_str ) - lv_len.

  " 뒤쪽 번호만 추출
  lv_num_part = lv_id_str+lv_start(lv_len).

  " 접두어 붙여서 최종 ID 생성
  ev_id = iv_prefix && lv_num_part.
