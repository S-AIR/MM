```abap
FUNCTION zc103mmfg0003.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(IV_PRID) TYPE  ZC103MMT0007-PRID
*"     REFERENCE(IV_VENDORID) TYPE  ZC103MMT0002-BPID
*"     REFERENCE(IV_APPROVAL) TYPE  ZC103FIT0011-EMPNO OPTIONAL
*"  EXPORTING
*"     REFERENCE(EV_POID) TYPE  ZC103MMT0011-POID
*"----------------------------------------------------------------------
  " 구매요청번호와 벤더아이디를 받아 구매오더로 전환하는 펑션모듈
  " 구매요청상태가 'B' 인 것만 전환한다

  DATA : gs_prhd       TYPE ZC103mmt0007,
         gs_prit       TYPE ZC103mmt0008,
         gt_prit       TYPE TABLE OF ZC103mmt0008,
         gs_pohd       TYPE ZC103mmt0011,
         gs_poit       TYPE ZC103mmt0012,
         gt_poit       TYPE TABLE OF ZC103mmt0012,
         gv_poid(8),
         gv_totalprice TYPE i,
         gv_index      TYPE sy-index.

  " 구매요청 가져오기
  SELECT SINGLE *
    FROM ZC103mmt0007
    INTO CORRESPONDING FIELDS OF gs_prhd
   WHERE prid = iv_prid.

  CHECK gs_prhd IS NOT INITIAL.

  CHECK gs_prhd-pr_status EQ 'C'.

  " 구매요청 아이템 가져오기
  SELECT *
    FROM ZC103mmt0008
    INTO CORRESPONDING FIELDS OF TABLE gt_prit
   WHERE prid = iv_prid.

  CHECK gt_prit IS NOT INITIAL.
  ...
  " 구매요청 아이템 구매오더로 전환
  LOOP AT gt_prit INTO gs_prit.

    gv_index = sy-index.

    gs_poit = CORRESPONDING #( gs_prit ).
    gs_poit = VALUE #( BASE gs_poit
                            poid = gv_poid
                            bpid = iv_vendorid
                            currency = 'KRW'
                            erdat = sy-datum
    ...
    gv_totalprice = gv_totalprice + gs_poit-price.
  ...
  " 구매요청헤더 구매오더헤더로 전환
  gs_pohd = CORRESPONDING #( gs_prhd ).
  gs_pohd = VALUE #( BASE gs_pohd
                          poid = gv_poid
                          order_date = gs_prhd-receive_date
                          receive_date = '00000000'
                          total_price = gv_totalprice
                          order_status = 'A'
                          approval = ''
                          currency = 'KRW'
                          approve_date = '00000000'
                          approve_time = '000000'
                          create_date = sy-datum
                          create_time = sy-uzeit
                          erdat = sy-datum
                          erzet = sy-uzeit ernam = sy-uname ).

  " 승인자 정보가 있을 경우 정보를 입력하고 오더 상태를 '승인' 상태로 설정한다
  IF iv_approval IS NOT INITIAL.

    gs_pohd-approval = iv_approval.
    gs_pohd-approve_date = sy-datum.
    gs_pohd-approve_time = sy-uzeit.
    gs_pohd-order_status = 'B'.

  ENDIF.

  " 디비에 반영
  INSERT ZC103mmt0011 FROM gs_pohd.
  INSERT ZC103mmt0012 FROM TABLE gt_poit.
